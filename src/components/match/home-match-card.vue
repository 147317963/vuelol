    ...
    <transition name="fade">
        <div>
            <template v-for="(item)  in matchList">
                <div class="home-match-card" :key="item.id" @click.stop=""
                    ...
                    <section class="card-footer">
                        <template v-for="(items,indexs)  in item['odds']">
                            ...
                            <div class="match-status" v-if="indexs===0">
                                <div :class="moment(item['start_time']).diff(moment()) <= 0 ? 'match-is-live' : 'match-is-early'">
                                    ...
                                    <div class="match-status-text">{{moment(item['start_time']).diff(moment()) <= 0 ?
                                        '滚盘中' : '未开始'}}
                                    </div>
                                </div>
                            </div>
                        </template>
                        <!--                        Object.assign(list[i], {selected:true})-->
                        <!--                        @click.stop=" $store.dispatch('match/updateMatcOddsList',{index:index,oddsIndex:0,data:Object.assign(item['odds'][0], {selected:!item['odds'][0]['selected'],odds:123})})"-->
                        <!--                        <div class="card-odds-btn"-->
                        <!--                             @click.stop="item['odds'].length >=1-->
                        <!--                             && item['odds'][0]['status'] !== oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']-->
                        <!--                             && item['odds'][0]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']-->
                        <!--                             ?item['odds'][0]['selected']=!item['odds'][0]['selected']:''">-->
                        <!--                            <div class="match-card-button"-->
                        <!--                                 :class="[-->
                        <!--                                     {-->
                        <!--                                         'btn-selected':item['odds'].length >=1 ?item['odds'][0]['selected'] && item['odds'][0]['status'] !== oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status'] && item['odds'][0]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:'',//选中-->
                        <!--                                         'btn-locked':item['odds'].length >=1 ?item['odds'][0]['status'] === oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']:'',//封盘中-->
                        <!--                                         'btn-over':item['odds'].length >=1 ?item['odds'][0]['status'] ===oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:true,//已结束-->
                        <!--                                     }-->
                        <!--                                     ]">-->
                        <!--                                <div class="button-dark-border">-->
                        <!--                                    <div class="button-content">-->
                        <!--                                        &lt;!&ndash;                                <div class="button-name">{{item['team'][0]['team_name']}}</div>&ndash;&gt;-->
                        <!--                                        <div class="button-name">-->
                        <!--                                            {{item['team'][_.findIndex(item['team'],{'pos':1})]['team_name']}}-->
                        <!--                                        </div>-->
                        <!--                                            <div class="button-odds-content" v-if="item['odds'].length >=1 ? item['odds'][0]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:false">-->
                        <!--                                                <div class="odds-rising-icon"></div>-->
                        <!--                                                <div class="btn-odds">-->
                        <!--                                                    <span v-if="item['odds'][0]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']">{{item['odds'][0]['odds']}}</span>-->
                        <!--                                                </div>-->
                        <!--                                                <div class="odds-dropping-icon"></div>-->
                        <!--                                            </div>-->
                        <!--                                    </div>-->
                        ...
                        <!--                        <div class="card-odds-btn"-->
                        <!--                             @click.stop="item['odds'].length >=2-->
                        <!--                             && item['odds'][1]['status'] !== oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']-->
                        <!--                             && item['odds'][1]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']-->
                        <!--                             ?item['odds'][1]['selected']=!item['odds'][1]['selected']:''">-->
                        <!--                            <div class="match-card-button"-->
                        <!--                                 :class="[-->
                        <!--                                     {-->
                        <!--                                         'btn-selected':item['odds'].length >=2 ?item['odds'][1]['selected'] && item['odds'][1]['status'] !== oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status'] && item['odds'][1]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:'',//选中-->
                        <!--                                         'btn-locked':item['odds'].length >=2 ?item['odds'][1]['status'] === oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']:'',//封盘中-->
                        <!--                                         'btn-over':item['odds'].length >=2 ?item['odds'][1]['status'] ===oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:true,//已结束-->
                        <!--                                     }-->
                        <!--                                     ]">-->
                        <!--                                <div class="button-dark-border">-->
                        <!--                                    <div class="button-content">-->
                        ...
                        <!--                                        <div class="button-odds-content"-->
                        <!--                                            v-if="item['odds'].length >=2 ? item['odds'][1]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已结算'})]['status']:false"-->
                        <!--                                        >-->


                        <!--                                            <div class="odds-rising-icon"></div>-->
                        <!--                                            <div class="btn-odds">-->
                        <!--                                                <span v-if="item['odds'][1]['status'] !==oddsStatus[_.findIndex(oddsStatus,{'name':'已封盘'})]['status']">{{item['odds'][1]['odds']}}</span>-->
                        <!--                                            </div>-->
                        <!--                                            <div class="odds-dropping-icon"></div>-->
                        <!--                                        </div>-->
                        <!--                                    </div>-->
...
<script>
    ...
    export default {
        ...
        methods: {//条用方法
            ...
            selected(item) {
                if (item['status'] !== this.oddsStatus[_.findIndex(this.oddsStatus, {'name': '已封盘'})]['status']
                    ...
                    _.findIndex(this.betList, {'id': item.id}) === -1 ? this.$store.dispatch('app/addBetList', Object.assign(item)) : this.$store.dispatch('app/deleteBetList', _.findIndex(this.betList, {'id': item.id}));
                }
            }
        },
        mounted() {//加载完毕后
        ...
        watch: {
            ...
            //
            //                         _.findIndex(this.betList,{'id':items.id}) ===-1? this.$store.dispatch('app/addBetList',Object.assign(items)):'';
            //
            //                     }else {
            //                         _.findIndex(this.betList,{'id':items.id}) ===-1 ?'':this.$store.dispatch('app/deleteBetList',_.findIndex(this.betList,{'id':items.id}))
            //                     }
            //
            //                 })
            //             })
