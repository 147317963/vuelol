                ...
                <div v-show="!matchRefresh">
                    <template v-for="(item)  in matchList">
                        <div class="home-match-card" :key="item.id"
                             v-show="Object.values(gameList).length && gameList[item['game_id']]['selected']">

                            <section class="card-header">
                                <!--                游戏logo-->
                                <img style="width: 20px"
                                     v-lazy="Object.values(gameList).length && Object.keys(configList).length ? configList['img_url']['value']+gameList[item['game_id']]['game_logo']:''">
                                <!--                跳转到当前赛季列表-->
                            ...
                            <section class="card-body">
                                <div class="card-body-team">
                                    <img class="team-logo"
                                         v-lazy="Object.values(gameList).length && Object.keys(configList).length ? configList['img_url']['value']+item['team'][_.findKey(item['team'],{'pos':1})]['team_logo']:''">
                                </div>
                                <div class="card-body-center">
                                    ...
                                    <div class="team-score" v-if="moment(item['start_time']).diff(moment())<=0">
                                        <div class="score left-score">
                                            {{item['team'][_.findKey(item['team'],{'pos':1})]['score']['total']}}
                                        </div>
                                        <div class="dash-symbol">-</div>
                                        <div class="score right-score">
                                            {{item['team'][_.findKey(item['team'],{'pos':2})]['score']['total']}}
                                        </div>
                                    </div>
                                    <!--                                开始时间-->
                                ...
                                <div class="card-body-team">
                                    <img class="team-logo"
                                         v-lazy="Object.values(gameList).length && Object.keys(configList).length ? configList['img_url']['value']+item['team'][_.findKey(item['team'],{'pos':2})]['team_logo']:''">
                                </div>
                            </section>
                            <div class="odds-group-title">
                                <template v-if="item['odds'].length">
                                    ...
                                    <div class="title">全场 {{item['odds'][0]['odds_group_name']}}</div>
                                </template>
                            ...
                            <section class="card-footer">
                                ...
                                <div class="card-odds-btn"
                                     @click.stop="selected(item['odds'][0])">
                                    <div class="match-card-button"
                                         :class="[
                                         addClass('btn-selected',item['odds'][0]),
                                         addClass('btn-locked',item['odds'][0]),
                                         addClass('btn-over',item['odds'][0]),
                                     ]">
                                        <div class="button-dark-border">
                                            <div class="button-content">
                                                ...
                                                <div class="button-odds-content"
                                                     v-if="item['odds'][0]? item['odds'][0]['status'] !==oddsStatus['已结束']['status']:false">
                                                    <!--                                                    >-->
                                                    <div class="odds-rising-icon"></div>
                                                    <div class="btn-odds">
                                                            <span
                                                                    v-if="item['odds'][0]? item['odds'][0]['status'] !==oddsStatus['已封盘']['status']:false">
                                                                {{item['odds'][0]['odds']}}
                                                            </span>
                                                    </div>
                                ...
                                <div class="match-status">
                                    <template v-if="item['status'] !==matchStatus['已结束']['status']">
                                    ...
                                    <template v-else-if="item['status'] ===matchStatus['已结束']['status']">
                                        <div  class="match-flag">
                                            <div   class="flag-icon" :class="[item['odds'][0]['win'] === winStatus['败']['status'] ?'flag-lose':'flag-win']">
                                                <span>{{winStatus[_.findKey(winStatus,{'status':item['odds'][0]['win']})]['name']}}</span>
                                            </div>
                                            <div   class="flag-icon" :class="[item['odds'][1]['win'] === winStatus['败']['status'] ?'flag-lose':'flag-win']">
                                                <span>{{winStatus[_.findKey(winStatus,{'status':item['odds'][1]['win']})]['name']}}</span>
                                            </div>
                                        </div>
                                ...
                                <div class="card-odds-btn"
                                     @click.stop="selected(item['odds'][1])">
                                    <div class="match-card-button"
                                         :class="[
                                         addClass('btn-selected',item['odds'][1]),
                                         addClass('btn-locked',item['odds'][1]),
                                         addClass('btn-over',item['odds'][1]),
                                     ]">
                                        <div class="button-dark-border">
                                            <div class="button-content">
                                                <!--                                <div class="button-name">{{item['team'][0]['team_name']}}</div>-->
                                                ...
                                                <!--                              !==oddsStatus['已结算']['status']          已经结算不会显示赔率-->
                                                <div class="button-odds-content"
                                                     v-if="item['odds'][1]? item['odds'][1]['status'] !==oddsStatus['已结束']['status']:false">
                                                    <!--                                                    >-->
                                                    <div class="odds-rising-icon"></div>
                                                    <div class="btn-odds">
                                                            <span
                                                                    v-if="item['odds'][1]? item['odds'][1]['status'] !==oddsStatus['已封盘']['status']:false">
                                                                {{item['odds'][1]['odds']}}
                                                            </span>
                                                    </div>
                                                    <div class="odds-dropping-icon"></div>
